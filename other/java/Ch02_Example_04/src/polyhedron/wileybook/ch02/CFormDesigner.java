...
import finobjects.Equity;
import finobjects.COption_BinAoN;
import finobjects.COption_BinCoN;
...
public class CFormDesigner extends Frame{
    ...
    /** form height */
    final int   frmHeight    	= frmWidth + yOffset;
    ...
    TextField txtAsset;
    /** tree step number input text box */
    TextField txtNrStep;
	...
	public CFormDesigner() {
        ...
        txtStrike 		= addTextBox("Strike Price",	  xOffset,3*yOffset,txtWidth);
        txtExpiry 		= addTextBox("Expiry (years)",	  xOffset,4*yOffset,txtWidth);
        txtRiskFree 	= addTextBox("Interest Rate (%)",2*xOffset + txtWidth,1*yOffset,txtWidth);
        txtNrStep 		= addTextBox("Step Number", 	2*xOffset + txtWidth,2*yOffset ,txtWidth);	
        // Combo boxes
        cmbOptionExercise	= addChoice("Option Exercise",	2*xOffset + txtWidth,3*yOffset ,txtWidth);
        cmbOptionExercise.add("EUROPEAN");
        cmbOptionExercise.add("AMERICAN");
        
        cmbOptionType	= addChoice("Option Payoff",	2*xOffset + txtWidth,4*yOffset ,txtWidth);
        cmbOptionType.add("CALL");
        cmbOptionType.add("PUT");
        // Output text area
	    textOutput 		= addTextArea("Price",xOffset,5*yOffset,xOffset + 2*txtWidth,true,false,false);
	    // buttons
        btnCalculate 	= addButton("Pricing",1,  xOffset,6*yOffset);
        btnQuit 		= addButton("Quit",   2,2*xOffset + 1*txtWidth,6*yOffset);
	}
    /*---------------------------------------------------------------------------------------------------------------------------------------*/
	/**
	...
	{
		/** set window title */
        setTitle("Your first Java Option Pricing Calculator");
    ...
    void btnCalculate_ActionPerformed(ActionEvent event) 
    {
    	...
    	String sOptionType		=   "";
    	/* a new object of type Equity is created */
    	Equity			Asset	=   new Equity();
    	/* a reference to an object of type COption_BinAoN (Binary Option
    	 * asset-or-nothing) is created */
    	COption_BinAoN  AoN;
    	/* a reference to an object of type COption_BinCoN (Binary Option
    	 * cash-or-nothin) is created */
    	COption_BinCoN  CoN;
    	try
    	{
    		...
    		Asset.setValue(dAssetPrice);
    		Asset.setVolatility(dAssetVolatility);
    		/* a new object of type COption_BinAoN is created, note the use of
    		 * the constructor with appropriate parameters
    		 */ 
    		AoN = new COption_BinAoN(sOptionType, dStrike, dExpiry, dRiskFreeRate, Asset);
    		/* a new object of type COption_BinCoN is created */ 
    		CoN = new COption_BinCoN(sOptionType, dStrike, dExpiry, dRiskFreeRate, Asset);
    		/* the plain vanilla option price is calcolated */
    		if(sOptionType == "CALL")
    			/* if the pv option is a 'call' then it can be replicated
    			 * with a long position in an asset-or-nothing and a short
    			 * position in a cash-or-nothing...
    			 */
    			dPrice	= 	AoN.Value() - CoN.Value();	
    		else
    			/* ...the opposite if the pv option is a 'put' */
    			dPrice 	=	CoN.Value() - AoN.Value();
