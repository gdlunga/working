...
from sphinx.ext.napoleon.docstring import _bullet_list_regex
#____________________________________________________________________________________________________
#
def rule_1(w, p):
    file_name = ''
...
#
dict_file_names = {} 
list_reg_exp = ['[0-9]{2}\.[0-9]{2}\.[0-9]{4}'
               ,'^[0-9]{4}_[0-9]{2}_[0-9]{2}'
               ,'[0-9]{2}[a-z]+[0-9]{4}'
               ,'[0-9]{2} [a-zA-Z]+ [0-9]{4}'
               ]
list_rules   = [rule_1,rule_2,rule_3,rule_4]

for w in onlyfiles:
    for r, f in zip(list_reg_exp, list_rules):
        p           = re.compile(r)    
        file_name   = f(w, p)
