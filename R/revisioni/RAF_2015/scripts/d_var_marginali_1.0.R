...
distribution <- function(name, scen, params){
  ...
  switch(name,
         "weibull" ={
           x <- rweibull (scen, shape= params[1], scale = params[2])
         },
         "normal" = {
...
file_ind  = paste("/output/param_distribuzioni_", versione, ".csv",sep="") 
param     = read.csv2(file=paste(g_OUTPUT_DIR,file_ind,sep="/"),1)

rownames(param)=param[,1]
param=param[2:ncol(param)]
