...
"""
	...
	def loadFromText(self, filename):
		...
		for chunk in chunks:
			tran = transac.tranFromString(chunk)
			self.add(tran)
		...
		for tran in self.__journal:
			for i in range(len(tran.lines)):
				...
				if mapping.has_key(account):
					tran.lines[i] = (mapping[account], amount, etc)
...
def test():
	...
	for trantuple in list:
		B3.add(transac.tranFromTuple(trantuple))
	if B.isEqual(B3):
